    ...
    describe("when `NODE_ENV=development`", () => {
      ...
      it("returns CSP directives flattened as a string", () => {
        ...
        expect(cspContent).toMatchInlineSnapshot(
          `"default-src 'none'; connect-src 'self'; font-src 'self'; frame-src 'self'; img-src 'self'; script-src 'self' 'unsafe-eval' 'sha256-qR24ahFUfDwOYNDgoxwK9PYcT5onKsO99GDSCLphspk='; style-src 'self' 'unsafe-inline'"`
        );
      });
    });
